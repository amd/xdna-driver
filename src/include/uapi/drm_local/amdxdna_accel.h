...
#define DRM_AMDXDNA_HWCTX_MEM_BITMAP		11
	__u32 param; /* in */
/*
 * Upper bound (sanity cap) on a single get_array element.  A full 24-column
 * AIE coredump is ~7 MB/column (~168 MB), so the cap must accommodate the
 * whole array; 256 MB leaves headroom.
 */
#define AMDXDNA_MAX_ELEMENT_SIZE		(256U * 1024 * 1024)
