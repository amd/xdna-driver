      ...
      if (e.code().value() == ENOBUFS) {
        ...
        try {
          ...
          if (retry_err.code().value() == EPERM) {
            throw std::runtime_error("No context has been scheduled yet. Please run a workload before requesting coredump.");
          } else if (retry_err.code().value() == EINVAL) {
            throw std::runtime_error("AIE coredump failed: Invalid hardware context");
          }
          throw;
        }
