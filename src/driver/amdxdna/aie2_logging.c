...
static u32 aie2_get_log_content(struct logging_req_buf *log_req_buf)
{
	...
	head_ptr = (u32)(log_metadata->head_offset_lo % LOG_RB_SIZE);
	tail_ptr = (u32)(log_metadata->tail_offset_lo);
	tail_ptr_wrap = tail_ptr % LOG_RB_SIZE;

	/* Update the Ring Buffer head pointer */
	log_metadata->head_offset_lo = tail_ptr;
...
static void aie2_print_log_buffer_data(struct amdxdna_dev_hdl *ndev)
{
        ...
        struct log_msg_header *log_content;
        u32 log_size;
        ...
        while (str < end) {
                log_content = (struct log_msg_header *)str;
                printk(KERN_INFO "FW log format: 0x%x level: 0x%x len: %u data: %s",
                       log_content->format, log_content->level, log_content->argc, (char*)(str+64));
                /*print_hex_dump_debug("Log data: ", DUMP_PREFIX_OFFSET, 16, 4, str,
                                     320, true);*/
                str += 320;
        }
}
