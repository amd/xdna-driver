...
static u32 aie2_get_log_content(struct logging_req_buf *req_buf)
{
	...
	log_rb_size = req_buf->dram_buffer_size - DRAM_LOG_BUF_METADATA_SIZE;
	WARN_ON(log_rb_size <= 0);

	drm_clflush_virt_range(sys_buf + log_rb_size, DRAM_LOG_BUF_METADATA_SIZE);
	memcpy(&log_metadata, sys_buf + log_rb_size, DRAM_LOG_BUF_METADATA_SIZE);
	...
	log_size = tail - head;
	XDNA_DBG(ndev->xdna, "log_size %u head %llu tail %llu",
		 log_size, head, tail);
...
static void aie2_print_log_buffer_data(struct amdxdna_dev_hdl *ndev)
{
	...
	while (str && str < end) {
		...
		str += msg_size;
		XDNA_INFO(ndev->xdna, "[NPU FW][%u]:%s", header->serial_num, msg);
		str = aie2_get_valid_msg_header(str, end);
	}
}
...
static int aie2_configure_and_start_logging(struct amdxdna_dev_hdl *ndev)
{
	...
	XDNA_DBG(xdna, "Set runtime log level: %u, ret %d", req_buf->log_level, ret);
			      ...
			      u32 buff_size, u32 log_level)
{
	...
	int ret = aie2_validate_dram_log_config(ndev, enable,
						   buff_size, log_level);
	if (ret)
		return;
