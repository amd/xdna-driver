			...
			void *buffer, u32 size)
{
	...
	for (int col = 0; col < nhwctx->num_col; ++col) {
		...
		for (int row = 0; row < MAX_ROW; ++row) {
			void *tile_addr = GET_TILE_ADDRESS(buffer, MAX_ROW, row, col);

			if (row == 0) {
				int ret = ve2_partition_read(aie_dev, rel_col, row, 0,
						TILE_ADDRESS_SPACE, tile_addr);
				XDNA_DBG(xdna,
					 "Read shim tile col:%d row:%d ret:%d",
					 rel_col, row, ret);
				if (ret < 0)
					return -EINVAL;
				...
				int ret1 = ve2_partition_read(aie_dev, rel_col, row, 0,
						MEM_TILE_MEMORY_SIZE, tile_addr);
				int ret2 = ve2_partition_read(aie_dev, rel_col, row,
						MEM_TILE_FIRST_REG_ADDRESS,
						TILE_ADDRESS_SPACE - MEM_TILE_FIRST_REG_ADDRESS,
						tile_addr + MEM_TILE_FIRST_REG_ADDRESS);
				XDNA_DBG(xdna,
					 "Read mem tile col:%d row:%d ret1:%d ret2:%d",
					 rel_col, row, ret1, ret2);
				if (ret1 < 0 || ret2 < 0)
					return -EINVAL;

			} else {
				int ret1 = ve2_partition_read(aie_dev, rel_col, row, 0,
						CORE_TILE_MEMORY_SIZE, tile_addr);
				int ret2 = ve2_partition_read(aie_dev, rel_col, row,
						CORE_TILE_FIRST_REG_ADDRESS,
						TILE_ADDRESS_SPACE - CORE_TILE_FIRST_REG_ADDRESS,
						tile_addr + CORE_TILE_FIRST_REG_ADDRESS);
				XDNA_DBG(xdna,
					 "Read core tile col:%d row:%d ret1:%d ret2:%d",
					 rel_col, row, ret1, ret2);
				if (ret1 < 0 || ret2 < 0)
					return -EINVAL;
			}
