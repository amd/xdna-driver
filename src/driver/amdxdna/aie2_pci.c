...
{
	const struct aie2_supported_fw_ver *supported = NULL;
	...
	/* Find matching major version in supported list */
	for (i = 0; i < ndev->priv->num_supported_fw_vers; i++) {
		if (fw_major == ndev->priv->supported_fw_vers[i].major) {
			supported = &ndev->priv->supported_fw_vers[i];
			break;
		}
	}

	if (!supported) {
		XDNA_ERR(xdna, "Unsupported firmware major version %d", fw_major);
		return -EINVAL;
	}

	if (fw_version < supported->min_fw_version) {
		XDNA_ERR(xdna, "Firmware %d.%d below minimum %d.%d", fw_major, fw_minor,
			 AIE2_FW_MAJOR(supported->min_fw_version),
			 AIE2_FW_MINOR(supported->min_fw_version));
		return -EINVAL;
	}
