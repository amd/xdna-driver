 ...
 *   };
 *
 * Partition geometry is not described in the device tree: each hardware
 * context creates its firmware partition at AIE4_MSG_OP_CREATE_PARTITION time
 * using the QoS start-column hint (or 0), and the firmware places it in the
 * client's slot.
 */
