...
{
	DECLARE_AIE4_MSG(aie4_attach_work_buffer, AIE4_MSG_OP_ATTACH_WORK_BUFFER);
	struct amdxdna_dev *xdna = ndev->xdna;
	int ret;

	if (size < AIE4_MPNPU_WORK_BUFFER_MIN_SIZE || !addr) {
...
{
	DECLARE_AIE4_MSG(aie4_detach_work_buffer, AIE4_MSG_OP_DETACH_WORK_BUFFER);
	struct amdxdna_dev *xdna = ndev->xdna;
	int ret;
