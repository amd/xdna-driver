...
// Submit a cmd with control code buf in a xrt::module
void
umq_cmd_submit(hwqueue_handle *hwq,
  cuidx_type::domain_index_type cu_idx, bo& exec_buf_bo, xrt::module& mod)
{
  if (cu_idx != 0)
  ...
  auto cmd_packet = reinterpret_cast<ert_start_kernel_cmd *>(exec_buf_bo.map());
  uint32_t *payload = reinterpret_cast<uint32_t *>(get_ert_dpu_data(cmd_packet));
  (void) xrt_core::module_int::fill_ert_dpu_data(mod, payload);
...
TEST_shim_umq_vadd(device::id_type id, std::shared_ptr<device> sdev, arg_type& arg)
{
  ...
  xrt_core::module_int::patch(mod, "g.ifm_ddr", 0, bo_ifm.get()->get_properties().paddr);
  xrt_core::module_int::patch(mod, "g.wts_ddr", 1, bo_wts.get()->get_properties().paddr);
  xrt_core::module_int::patch(mod, "g.ofm_ddr", 0, bo_ofm.get()->get_properties().paddr);
...

  {
    ...
    for (int i = 0; i < 1; i++) {
      ...
      umq_cmd_submit(hwq, cu_idx.index, bo_exec_buf, mod);
