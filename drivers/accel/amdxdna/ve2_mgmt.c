...
static void ve2_free_hs_data(struct aie_op_handshake_data *hs_data, u32 max_cols)
{
	 ...
	 * pre-allocated handshake buffer (freed in ve2_hwctx_fini) or allocated
	 * contiguously with the descriptor array in the fallback path. In both
	 * cases a single kfree() of the descriptor array is sufficient.
	 */
	kfree(hs_data);
}
...
ve2_prepare_hs_data(struct amdxdna_mgmtctx *mgmtctx, struct amdxdna_hwctx *hwctx, bool init)
{
	...
	if (vp && vp->hs_buf_va) {
		/* Fast path: carve each cert_hs out of the pre-allocated buffer. */
		...
		size_t total_size = num_col * sizeof(*hs_data) + num_col * col_size;
...
static bool ve2_check_context_req(struct amdxdna_mgmtctx *mgmtctx)
{
	...
	/*
	 * Only mark the partition idle-due-to-context (which drives a
	 * reprogram/switch) when the command FIFO head actually belongs to a
	 * different context than the active one. Otherwise the switch would be
	 * a redundant reprogram of the already-active context.
	 */
