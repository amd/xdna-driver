...
static void aie2_tdr_recover_all(struct amdxdna_dev *xdna)
{
	...
	amdxdna_for_each_client(xdna, client)
		amdxdna_hwctx_walk(client, &fw_dead, NULL, aie2_tdr_stop_hwctx);

	if (fw_dead) {
		int ret;

		/*
		 * Firmware did not respond — it is wedged. Power-cycle the NPU
		 * via SMU to reload the firmware. aie2_hw_reset() handles full
		 * device recovery including context restart, so skip the
		 * per-client resume below.
		 * aie2_hw_reset() rate-limits itself to one reset per
		 * AIE2_HW_RESET_MIN_INTERVAL_MS; if the reset is skipped
		 * (-EAGAIN), resume the paused contexts below so they are
		 * not left permanently stalled.
		 */
		XDNA_WARN(xdna, "Firmware unresponsive, power-cycling NPU");
		ret = aie2_hw_reset(xdna);
		if (!ret)
			return;
	}
