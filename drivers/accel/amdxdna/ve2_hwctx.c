...
MODULE_PARM_DESC(ve2_perf_optimization, "Enable performance-mode partition init options. Disabled by default.");
...
int ve2_hwctx_init(struct amdxdna_hwctx *hwctx)
{
	...
	/*
	 * Pre-allocate a handshake buffer covering all columns of this
	 * partition. ve2_prepare_hs_data() carves per-column handshake structs
	 * out of it, avoiding a per-column allocation on every partition
 ...
 *
 * Queue-full handling: this driver retries per-command via
 * submit_command_retry() (submit -> on -EBUSY, wait on ve2_wait_for_retry_slot()
 * for a completion IRQ to free a slot, then retry). This is functionally
 * equivalent to the legacy out-of-tree ve2_submit_cmd_chain_partial() batch
 * loop (same wait, same VE2_RETRY_TIMEOUT_MS, same "every command is submitted"
 * guarantee), just structured per-command instead of per-batch.
 *
 * The legacy partial-submit path is intentionally NOT ported: on a mid-chain
 * timeout it registered a *partial* job (ve2_hwctx_add_job(..., total_submitted)
 * + ERT_CMD_STATE_TIMEOUT) to reap already-queued sub-commands. Here
 * ve2_hwctx_job_release() recomputes the completion count from the *full*
 * cmd_chain->command_count in the BO payload, so adding a partial job would
 * desync vp->submitted against the reap path. Instead, a mid-chain failure
 * returns the error and the whole job is failed by the upper layer.
 */
