...
int amdxdna_drm_destroy_hwctx_ioctl(struct drm_device *dev, void *data, struct drm_file *filp)
{
	 ...
	 * data, and there is no message to release it on its own. Resume before
	 * the erase below, while the ctx is still visible to the resume walk, so
	 * it is recreated and the destroy reaches firmware. Best effort only: a
	 * ctx must always be destroyable, so a failed resume still destroys it
	 * and leaves the pool entry behind until the next suspend or reset.
	 */
